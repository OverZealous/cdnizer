...
module.exports = util = {
	...
	matchers: [
		...
		{ pattern: /(<link\s[\s\S]*?href=["'])([\s\S]+?)(["'][^>]*>(?:\s*<\/link>)?)/gi, fallback: true },
		// link with unquoted href and optional end tag
		{ pattern: /(<link\s[\s\S]*?href=)([\s\S]+?)((?:>|\s[^>]*>)(?:\s*<\/link>)?)/gi, fallback: true },

		// img with quoted src
		{ pattern: /(<img\s[\s\S]*?src=["'])(.+?)(["'][^>]*>)/gi, fallback: false },
		// img with unquoted src
		{ pattern: /(<img\s[\s\S]*?src=)(.+?)((?:>|\s[^>]*>))/gi, fallback: false },
	...
	getVersionInfo: function(fileInfo, opts) {
		...
		if(!opts._versionInfoCache[pkg]) {
			var packageInfo, version, versionClean, packageRoot = path.join(process.cwd(), getBowerRoot(opts), pkg);
			if(fileInfo.version) {
				packageInfo = fileInfo;
